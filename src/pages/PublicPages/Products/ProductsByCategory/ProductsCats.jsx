        ...
        <div className="relative mx-auto block h-full bg-white container-wrapper w-full">
          <div className="w-full md:flex md:gap-2">
            <LeftSection
              ...
              // tertiaryCats={tertiaryCats}
            />
            {/* aside left */}
            <ProductSection params={params} category={category}>
              <ProductItem params={params} category={category} />
            </ProductSection>
...
const LeftSection = ({ params, subcategories }) => {
  return (
...
const ProductSection = ({ children, params, category }) => {
          ...
          <div className="flex flex-wrap">
            {category.subcategory &&
              category.subcategory.slice(0, 4).map((subcategory) => (
                <Link
                  to={`/product/subcategory/${subcategory.name}`}
                  key={subcategory.id}
            ...
            {showMore && (
              <>
                {category.subcategory &&
                  category.subcategory.slice(4).map((subcategory) => (
                    <Link
                      to="#"
                      key={subcategory._id}
                      className="inline-block border text-xs capitalize rounded-[3px] px-2xs ml-2xs h-[28px] leading-[28px] mb-[15px] border-dark-gray"
...
const ProductItem = ({ toggleProduct, params, category }) => {
  ...
  const fetchProducts = async (page = 1, limit = 20) => {
    const response = await apiClient.get(
      `${endpoints.getAllPosts}?category=${category.name}&page=${page}&limit=${limit}`
    );
